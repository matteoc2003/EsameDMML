...
> **Nota**: I file CSV originali non sono inclusi nella repository per via delle loro dimensioni (~500 MB totali).  
...
| **Features chiave**     | `amt`, `category`, `gender`, `lat`, `long`, `merch_lat`, `merch_long`, `unix_time`, `city_pop` |
...
- Le transazioni fraudolente hanno **importo mediano più alto** rispetto alle legittime, ma non raggiungono i picchi massimi → i truffatori operano in una fascia di importo specifica
...
- Il modello migliore (Random Forest) è addestrato su dati bilanciati (30% frodi), diversi dalla distribuzione reale (~0.57%)
...
| **Naive Bayes**       | Medio         | ~4.600 falsi allarmi extra rispetto al GLM     |
| **KNN (k=10)**        | Variabile     | Sensibile alla normalizzazione                 |
| **GLM**               | Solido        | ASE Test < ASE Train, nessun overfitting       |
...
- La ricalibrazione Bayesiana è essenziale: senza di essa le probabilità risulterebbero sistematicamente sovrastimate di circa 50x rispetto alla realtà
...
```
TesinaML/
├── fraudSample.csv      # 10 righe di esempio (incluso nel repo)
├── Tesina.Rmd           # Report principale (sorgente R Markdown)
└── Tesina-copy.docx     # Report esteso compilato (Word)
```

I file `fraudTrain.csv` e `fraudTest.csv` vanno scaricati da Kaggle (link sopra).
...
| Libreria       | Utilizzo                                           |
|----------------|----------------------------------------------------|
| `ROSE`         | Bilanciamento classi                               |
| `caret`        | Training unificato, CV, preprocessing              |
...
2. Scaricare i dataset da Kaggle e posizionarli nella cartella `TesinaML/`
3. Installare le librerie necessarie:
   ...
   install.packages(c("ROSE", "caret", "rpart", "randomForest", "gbm",
                      ...
                      "ggplot2", "dplyr"))
   ```
4. Aprire `Tesina.Rmd` in RStudio e premere **Knit** per generare il report
